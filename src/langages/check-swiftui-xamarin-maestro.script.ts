...
async function checkSwiftUIXamarinMaestro() {
  const app = await NestFactory.createApplicationContext(AppModule);

  try {
    const langageModel = app.get<Model<any>>('LangageModel');

    console.log('🔍 VÉRIFICATION DE SWIFTUI, XAMARIN ET MAESTRO\n');
    console.log('═══════════════════════════════════════════════════════════\n');

    const items = ['SwiftUI', 'Xamarin', 'Maestro'];
...
checkSwiftUIXamarinMaestro().catch(error => {
