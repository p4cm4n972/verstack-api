...
async function checkVariousAnomalies() {
  const app = await NestFactory.createApplicationContext(AppModule);

  try {
    const langageModel = app.get<Model<any>>('LangageModel');

    console.log('🔍 VÉRIFICATION DES ANOMALIES DIVERSES\n');
    console.log('═══════════════════════════════════════════════════════════\n');

    const items = ['SQL', 'Perl', 'Haskell', 'Apache Spark', 'Jupyter', 'Polars', 'Seaborn'];
...
checkVariousAnomalies().catch(error => {
