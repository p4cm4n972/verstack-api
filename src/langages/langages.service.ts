...
constructor(@InjectModel(Langage.name) private readonly langageModel: Model<Langage>) {}
    ...
    async update(id: string, updateLangageDto: any) {
        const existingLangage = await this.langageModel.findOneAndUpdate({_id: id}, {$set: updateLangageDto}, {new: true}).exec();
        if (!existingLangage) {
            throw new NotFoundException(`Langage with id ${id} not found`);
        }
